...
def build_laplacian_pyramid(image, levels):
    """
    image: np.ndarray, shape (H, W) or (1, H, W)
    returns: list of np.ndarrays, each with shape (1, h, w)
    """
    if image.ndim == 3 and image.shape[0] == 1:
        image = image[0]  # shape (H, W)

    image = image.astype(np.float32)
    gaussian_pyramid = [np.expand_dims(image, axis=0)]

    for _ in range(levels):
        image = cv2.pyrDown(image)
        gaussian_pyramid.append(np.expand_dims(image, axis=0))
