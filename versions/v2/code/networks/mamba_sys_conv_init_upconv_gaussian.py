...
class GaussianBlur(nn.Module):
    def __init__(self, kernel_size=5, sigma=1.0, down_factor=2):
        super().__init__()
        ...
        # 创建高斯核并注册为 buffer
        kernel = self.create_gaussian_kernel(kernel_size, sigma)  # [1, 1, k, k]
        self.register_buffer("base_kernel", kernel)  # <- 这里

    def create_gaussian_kernel(self, k, s):
        ax = torch.arange(k) - k // 2
        xx, yy = torch.meshgrid(ax, ax, indexing="ij")
        kernel = torch.exp(-(xx**2 + yy**2) / (2 * s**2))
        kernel /= kernel.sum()
        return kernel.view(1, 1, k, k)

    def forward(self, x):
        C = x.size(1)
        kernel = self.base_kernel.repeat(C, 1, 1, 1)
        return F.conv2d(x, kernel, padding=self.padding, stride=self.down_factor, groups=C)
...
class WLBlock(nn.Module):
    def __init__(self, dim):
        ...
        self.pool = GaussianBlur(kernel_size=5, sigma=1.2)  # 使用高斯模糊代替 DWT

    def forward(self, x):
        x = self.conv(x)
        low = self.pool(x)  # 使用平均池化代替 DWT
