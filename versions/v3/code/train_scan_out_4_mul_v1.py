...
from dataloaders.multi_brain_dataset import SliceData_multi
...
def main_worker(args):
    ...
    if args.model == "mamba_unrolled":
        from networks.vision_mamba_mul_v1 import MambaUnrolled as VIM_seg
    ...
    # 数据集与采样器
    train_dataset = SliceData_multi(
        data_dir="/scratch/pf2m24/data/M4Raw/M4RawV1.5_multicoil_train/multicoil_train",
        mask_path="/scratch/pf2m24/projects/MRIRecon/MambaReconV3/code/dataloaders/multi_mask/mask_0_256_af4.npy"
    )
    
    val_dataset = SliceData_multi(
        data_dir="/scratch/pf2m24/data/M4Raw/M4Raw_multicoil_test/multicoil_test",
        mask_path="/scratch/pf2m24/projects/MRIRecon/MambaReconV3/code/dataloaders/multi_mask/mask_0_256_af4.npy"
    )
    ...
    for epoch in range(start_epoch, max_epoch + 1):
        ...
        for i_batch, sampled_batch in enumerate(pbar):
            ...
            outputs = torch.abs(outputs[:, :C//2, :, :] + 1j * outputs[:, C//2:, :, :])
            outputs = torch.sqrt(torch.sum(outputs ** 2, dim=1, keepdim=True))
            ##############################
            outputs = outputs / (outputs.amax(dim=(-2, -1), keepdim=True) + 1e-8)
            ########################
...
if __name__ == "__main__":
    ...
    #parser.add_argument('--resume', default='/scratch/pf2m24/projects/MRIRecon/MambaReconV3/model/mamba_unrolled_140_Patch_1_mul_4_P1_v1/mamba_unrolled/mamba_unrolled_latest_model.pth')
    ...
    parser.add_argument('--name', type=str, default='Mulit_8')
