...
def list_nii_files(path):
    ...
    print("total patient num:", n)
    return nii_files


def center_crop_224(arr: np.ndarray) -> np.ndarray:
    """
    从任意 2D numpy 数组中裁出中心的 224×224 区域。
    若原图比 224 小，会自动补零。
    """
    if arr.ndim != 2:
...
class SliceData_pro(Dataset):
    def __init__(self, 
                data_dir="/scratch/pf2m24/data/prostate158/prostate158_test/test", mask_path="/scratch/pf2m24/projects/MRIRecon/MambaReconV3/code/dataloaders/random_mask/random_224_4.npy"):
        ...
        self.coil_map = np.ones([1,224,224], dtype=np.float32)
        self.re = torchvision.transforms.Resize((224,224))
    def __len__(self):
    ...
    def __getitem__(self, i):
        ...
        # image_rec = center_crop_224(dataA[slice_num])
        image_rec = torch.from_numpy(dataA[slice_num])
        image_rec = self.re(image_rec.unsqueeze(0).unsqueeze(0)).squeeze().numpy()
        ...
        # return under_image_rec, image_rec, mask, torch.from_numpy(self.coil_map)
...
class Unet_ACDC(Dataset):
    ...
    def __getitem__(self, i):
        ...
        dataA = sitk.ReadImage(fname)
        dataA = sitk.GetArrayFromImage(dataA).astype(np.float32)
        dataA = center_crop_224(dataA[slice_num])
...
class Ista_ACDC(Dataset):
    ...
    def __getitem__(self, i):
        ...
        image_rec = (image_rec - torch.min(image_rec)) / (torch.max(image_rec) - torch.min(image_rec))
...
if __name__ == "__main__":
    ...
    for i, sampled_batch in enumerate(tqdm(data_loader)):
        us_image, fs_image, us_mask, coil_map = sampled_batch
        print(us_image.shape, fs_image.shape, us_mask.shape, coil_map.shape)
        if i == 1:
            break
