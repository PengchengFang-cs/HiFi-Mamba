...
class Unpatchify(nn.Module):
    def __init__(self, dim, dim_scale=4, norm_layer=nn.LayerNorm):
        ...
        self.dim = dim
        self.dim_scale = dim_scale
        self.layer = nn.Linear(dim, 8 * dim_scale**2, bias=False)
...
class VSSBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        low = self.drop_path(self.self_attention(self.ln_1(low), self.ln_2(high)))
...
class DataConsistency(nn.Module):
    def __init__(self, num_of_feat_maps, patchify, patch_size=2):
        ...
        self.patch_embed = PatchEmbed2D(patch_size=patch_size, in_chans=8, embed_dim=num_of_feat_maps, norm_layer=nn.LayerNorm)

    def data_cons_layer(self, im, mask, zero_fill, coil_map):

        _, C, H, W =zero_fill.shape
        im_complex = im[:,:C//2,:,:]+ 1j * im[:,C//2:,:,:]
        
        zero_fill_complex = zero_fill[:,:C//2,:,:] + 1j * zero_fill[:,C//2:,:,:]

        actual_kspace = fft2c(zero_fill_complex)
        gen_kspace = fft2c(im_complex)
        mask_bool = mask>0
        mask = torch.tile(mask_bool, dims=[1,C//2,1,1])
        gen_kspace_dc = torch.where(mask, actual_kspace, gen_kspace)
        gen_im = ifft2c(gen_kspace_dc)
        gen_im_return = torch.cat([torch.real(gen_im), torch.imag(gen_im)], dim=1)
...
class VSSM_unrolled(nn.Module): 
    def __init__(self, patch_size=4, in_chans=8, num_classes=2, depths=[2, 2, 2, 2, 2, 2], 
