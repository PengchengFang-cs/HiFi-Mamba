...
class WLBlock(nn.Module):
    def __init__(self, dim):
        ...
        self.conv = DWInvertedBottleneck(in_channels=dim, out_channels=dim, expansion_ratio=2)
...
class SharedRoutedMoE_BHWC(nn.Module):
    ...
    @torch.no_grad()
    def _balance(self, gates):
        # gates: [B,H,W,E]
        importance = gates.mean(dim=(0,1,2))                           # [E]
        load = (gates > 0).to(gates.dtype).mean(dim=(0,1,2))           # [E]
        return (importance * load).mean()
    ...
    def forward(self, x):
        ...
        # 轻量：默认 top-1（最稳）
        # if self.top_k == 1:
        #     top_idx = gates.argmax(dim=-1)                            # [B,H,W], 每像素选1个专家
        #     routed_out = torch.zeros_like(x)

        #     # 预先计算所有专家的输出，避免重复前向
        #     expert_outputs = [exp(x) for exp in self.experts]         # list of [B,H,W,C]

        #     for eid, eout in enumerate(expert_outputs):
        #         mask = (top_idx == eid).unsqueeze(-1)                 # [B,H,W,1]
        #         routed_out = torch.where(mask, eout, routed_out)      # 选择该专家输出
        if self.top_k == 1:
            B, H, W, C = x.shape
            ...
            x_flat = x.reshape(B * H * W, C)            # [N, C], N = B*H*W
            top_idx_flat = top_idx.reshape(-1)          # [N]

            # 3. 预先分配输出缓冲区
            routed_flat = torch.empty_like(x_flat)      # [N, C] 或 [N, C_out]
            ...
            for eid, expert in enumerate(self.experts):
                ...
                if not mask.any():
                    continue                            # 这个专家没被选中，跳过

                # 取出属于该专家的 tokens
                x_e = x_flat[mask]                      # [N_e, C]

                # 只在这 N_e 个 token 上跑一次该 expert
                y_e = expert(x_e)                       # [N_e, C] or [N_e, C_out]

                # 将结果 scatter 回全局位置
                routed_flat[mask] = y_e

            # 5. reshape 回去
            routed_out = routed_flat.reshape(B, H, W, -1)
        else:
            # top-2（或更大）带权重融合，依旧不展平
            top_val, top_idx = gates.topk(self.top_k, dim=-1)         # [B,H,W,K]
            routed_out = torch.zeros_like(x)
            # 预先计算所有专家的输出
            expert_outputs = [exp(x) for exp in self.experts]         # list of [B,H,W,C]
            for kpos in range(self.top_k):
                idx_k = top_idx[..., kpos]                            # [B,H,W]
                w_k   = top_val[..., kpos].unsqueeze(-1)              # [B,H,W,1]
                tmp = torch.zeros_like(x)
                for eid, eout in enumerate(expert_outputs):
                    mask = (idx_k == eid).unsqueeze(-1)               # [B,H,W,1]
                    tmp = torch.where(mask, eout, tmp)
                routed_out = routed_out + w_k * tmp
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.down1 = nn.Linear(hidden_dim*2, hidden_dim) 
        self.down2 = nn.Linear(hidden_dim*4, hidden_dim*2) 
    ...
    def forward(self, input: torch.Tensor):
        ...
        high, low = torch.split(global_features, C // 2, dim=-1)
        # global_features = self.down1(global_features)
        # token聚合
        # tokens = torch.cat([global_features, high, low], dim=0)
        # moe_tokens = self.moe(tokens.permute(0, 3, 1, 2))
        # # split back
        # global_features = moe_tokens[:B]
        # rest = moe_tokens[B:]

        # high = rest[:high.size(0)]
        # low = rest[low.size(0):]
        ...
        # 三路分别通过同一 MoE（共享专家池）
        # global_features, bal_g = self.moe(global_features)               # global 路
        # high, bal_y = self.moe(high)                  # y 路
        # low, bal_z = self.moe(low)                  # z 路
        # x = torch.cat([global_features, output_high, low], dim=-1)
        
        # balance_loss = bal_g + bal_y + bal_z
        ...
        #     plot_grid_8(spec14_b, spec14_a, save_path=os.path.join('/scratch/pf2m24/projects/MRIRecon/MambaRecon/inference_results/hifimamba', "figA_freq_grid.png"))

        #     # 4) 图B：径向 log 幅值曲线（可加 show_delta=True 看差值）
        #     plot_radial_curve(mag_b, mag_a, save_path=os.path.join('/scratch/pf2m24/projects/MRIRecon/MambaRecon/inference_results/hifimamba', "figB_radial.png"), show_delta=False)
...
class VSSM_unrolled(nn.Module): 
    def __init__(self, patch_size=4, in_chans=2, num_classes=2, depths=[2, 2, 2, 2, 2, 2, 2, 2], 
                 hidden_dim=96, d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
