...
def _ratio_split_indices(C: int, ratio: float = 0.5):
    """返回按比例切分的前半通道数 k。"""
    k = int(round(C * ratio))
    k = max(1, min(C - 1, k))  # 避免 0 或 C
    return k

class ButterflyMixerSplit(nn.Module):
    """
    多阶段“通道对”线性混合，形似 FFT 蝴蝶图：
    每个 stage 对若干 (even, odd) 通道对做 2x2 线性变换，堆叠 log2(C_pad) 个 stage。
    最后固定切分前 k/后 C-k 通道。
    - 若 C 不是 2 的幂，则 pad 到 2^m，再裁回。
    - 参数量 ~ 4 * (C_pad/2) * log2(C_pad) = O(C log C)。
    """
    def __init__(self, C: int, ratio: float = 0.5, ortho_init: bool = True):
        super().__init__()
        self.C = C
        self.C_pad = _next_pow2(C)
        self.ratio = ratio
        stages = int(math.log2(self.C_pad))

        # 每个 stage 有 (C_pad/2) 个 2x2 权重矩阵
        self.weights = nn.ParameterList()
        for _ in range(stages):
            # shape: [C_pad//2, 2, 2]
            W = nn.Parameter(torch.empty(self.C_pad // 2, 2, 2))
            if ortho_init:
                # 初始化为近似正交的 2x2
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.C_pad != C:
            pad = self.C_pad - C
            x = F.pad(x, (0, 0, 0, 0, 0, pad), value=0.0)  # [B, C_pad, H, W]

        # 2) 展平成 [N, C_pad]，N=B*H*W
        x_ = x.permute(0, 2, 3, 1).contiguous().view(-1, self.C_pad)  # [N, C_pad]
        N = x_.shape[0]

        # 3) 逐 stage 蝴蝶混合（用逐元素广播代替 matmul）
        size = 1
        for W2x2 in self.weights:  # W2x2: [C_pad//2, 2, 2]
            block = size * 2

            # [N, C_pad] -> [N, C_pad//block, block]
            x_ = x_.view(N, self.C_pad // block, block)

            # 切成 even / odd 半块
            even = x_[..., :size]          # [N, C_pad//block, size]
            odd  = x_[..., size:block]     # [N, C_pad//block, size]

            # 把 (even, odd) 展成 pair 维度：pair_count = (C_pad//block)*size == C_pad//2
            pair_count = (self.C_pad // block) * size
            # [N, pair_count]
            E = even.reshape(N, pair_count)
            O = odd .reshape(N, pair_count)

            # 取出 2×2 的四个标量权重通道（会在维度 0 上对 N 广播）
            # a,b,c,d: [pair_count]
            a = W2x2[:, 0, 0]  # 乘 E → 去 even
            b = W2x2[:, 0, 1]  # 乘 O → 去 even
            c = W2x2[:, 1, 0]  # 乘 E → 去 odd
            d = W2x2[:, 1, 1]  # 乘 O → 去 odd

            # 4) 逐元素 fused 计算（避免构造 [N,pair_count,2,1]）
            # Ye_flat / Yo_flat: [N, pair_count]
            Ye_flat = E * a.unsqueeze(0) + O * b.unsqueeze(0)
            Yo_flat = E * c.unsqueeze(0) + O * d.unsqueeze(0)

            # 还原回两个半块，并拼回当前 stage 的输出
            Ye = Ye_flat.view(N, self.C_pad // block, size)
            Yo = Yo_flat.view(N, self.C_pad // block, size)
            x_ = torch.cat([Ye, Yo], dim=-1).reshape(N, self.C_pad)

            size *= 2

        # 4) 折回 [B, C_pad, H, W]
        x = x_.view(B, H, W, self.C_pad).permute(0, 3, 1, 2).contiguous()

        # 5) 裁回原 C
        if self.C_pad != self.C:
            x = x[:, :self.C]

        # 6) 固定切分：前 k -> A 路，后 C-k -> B 路
        k = _ratio_split_indices(self.C, self.ratio)
        xA, xB = x[:, :k], x[:, k:]
        return xA, xB

class ShuffleMixerSplit(nn.Module):
    def __init__(self, C, ratio=0.5, groups=8, hidden_scale=1.0):
        super().__init__()
        hidden = int(C * hidden_scale)
        self.mix = nn.Sequential(
            nn.Conv2d(C, hidden, 1, groups=groups, bias=False),
            nn.GELU(),
            nn.Conv2d(hidden, C, 1, groups=groups, bias=False),
        )
        self.ratio = ratio
    def forward(self, x):
        z = self.mix(x)                              # 结构化近似旋转
        # 可选：channel shuffle：z = z.view(B, groups, C//groups, H, W).transpose(1,2).reshape(B,C,H,W)
        k = _ratio_split_indices(z.shape[1], self.ratio)
        return z[:, :k], z[:, k:]
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.idx = id
        self.split = ShuffleMixerSplit(hidden_dim*2)
    ...
    def forward(self, input: torch.Tensor):
        ...
        # if self.idx % 2 == 1:
        #     input = torch.flip(input, dims=(-2, -1)).contiguous()
        # if self.idx % 2 == 0:
        #     input = torch.transpose(input, dim0=-2, dim1=-1).contiguous()

        # # input = self.local(input)  # (B, H, W, C)
        # input = input.permute(0, 2, 3, 1) 

        C = input.shape[-1]
        # print("input shape", input.shape)
        # high, low = torch.split(input, C // 2, dim=-1)
        high, low = self.split(input.permute(0, 3, 1, 2))  # (B, C, H, W)
...
if __name__ == "__main__":
    ...
    x = torch.randn((4, 2, 320, 320)).cuda()
    us_im = torch.randn((4, 1, 320, 320)).to(torch.complex64).cuda()
    us_mask = torch.randn((4, 1, 320, 320)).cuda()
    coil_map = torch.ones((4, 1, 320, 320)).cuda()
