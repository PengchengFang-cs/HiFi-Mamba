...
class SS2D(nn.Module):
    ...
    def build_2d_rope(self, H, W, dim, device=None):
        """
        Build 2D rotary position embedding for [H, W] grid.
        Args:
            H, W: height and width of the feature map
            dim: total embedding dim (must be divisible by 4)
        Returns:
            sin_y, cos_y, sin_x, cos_x: each is [1, H, 1, dim/4] or [1, 1, W, dim/4]
        """
        assert dim % 4 == 0, "RoPE dim must be divisible by 4"
        half_dim = dim // 4

        inv_freq = 1.0 / (10000 ** (torch.arange(half_dim, dtype=torch.float32, device=device) / half_dim))

        y_pos = torch.arange(H, device=device).float()
        x_pos = torch.arange(W, device=device).float()

        sin_y = torch.sin(torch.outer(y_pos, inv_freq))  # [H, dim/4]
        cos_y = torch.cos(torch.outer(y_pos, inv_freq))  # [H, dim/4]
        sin_x = torch.sin(torch.outer(x_pos, inv_freq))  # [W, dim/4]
        cos_x = torch.cos(torch.outer(x_pos, inv_freq))  # [W, dim/4]

        # reshape to broadcast
        sin_y = sin_y.view(1, H, 1, half_dim)
        cos_y = cos_y.view(1, H, 1, half_dim)
        sin_x = sin_x.view(1, 1, W, half_dim)
        cos_x = cos_x.view(1, 1, W, half_dim)

        return sin_y, cos_y, sin_x, cos_x


    def apply_2d_rope(self, x, sin_y, cos_y, sin_x, cos_x):
        """
        Apply 2D rotary position embedding to x: [B, C, H, W]
        C must be divisible by 4
        """
        B, C, H, W = x.shape
        assert C % 4 == 0, "Channel dimension must be divisible by 4 for RoPE"

        x = x.permute(0, 2, 3, 1)  # [B, H, W, C]
        c_quarter = C // 4

        # Split into 4 parts
        x1 = x[..., 0:c_quarter]            # Y_even
        x2 = x[..., c_quarter:2*c_quarter]  # Y_odd
        x3 = x[..., 2*c_quarter:3*c_quarter]# X_even
        x4 = x[..., 3*c_quarter:]           # X_odd
        ...
        out = torch.cat([y_rot, x_rot], dim=-1)  # [B, H, W, C]
...
class VSSBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        high_branch_low = high - output_high
        low = low.permute(0, 2, 3, 1)
        high_branch_low=self.simple_gate(high_branch_low).permute(0, 2, 3, 1)

        low = self.drop_path(self.self_attention(self.ln_1(low), self.ln_2(high_branch_low)))

        x = torch.cat([output_high.permute(0,2,3,1), low], dim=-1)
        # x = input + x

        x = self.channel_attention(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1) + input
        # x = x.permute(0, 3, 1, 2)  # (B, C, H, W)
        
        # x = x + self.lem(x)  # LEM layer
        # x = input + self.drop_path(self.self_attention(self.ln_1(input), self.ln_2(high_branch_low)))
