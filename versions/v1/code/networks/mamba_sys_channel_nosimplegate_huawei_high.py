...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.high_freq_enhancer = nn.Sequential(
            nn.Conv2d(hidden_dim, hidden_dim, kernel_size=7, padding=3, bias=False),
    ...
    def forward(self, input: torch.Tensor):
        x = input.permute(0, 3, 1, 2)  # (B, C, H, W)
        x = self.local(x).permute(0, 2, 3, 1)  # (B, H, W, C)
        ...
        # high_blur = F.avg_pool2d(high, kernel_size=3, stride=1, padding=1)
        # high_diff = high - high_blur  # 差分代表高频部分

        # 卷积建模高频信息
        high_enhanced = self.high_freq_enhancer(high)

        # 与 WLBlock 的 high_hidden_features 融合（可以简单加）
        high = high_enhanced + high_hidden_feautres
        ...
        x = torch.cat([output_high.permute(0,2,3,1), low], dim=-1) #+ input
        # x = input + x
