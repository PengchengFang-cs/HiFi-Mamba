...
class ChannelAttentionBlock(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.conv3x3 = nn.Conv2d(in_channels, in_channels, kernel_size=1)
        self.relu = nn.GELU()
...
class SS2D(nn.Module):
    ...
    def forward_core(self, x: torch.Tensor, high_branch_low: torch.Tensor):
        ...
        xs = x_hwwh
        #####################################
        high_branch_low_seq = high_branch_low.view(B, 1, -1, L).contiguous()
        ...
        # dts_low = self.simple_gate_dts(dts)
    ...
    def forward(self, x: torch.Tensor, high_branch_low: torch.Tensor,**kwargs):
        ...
        y = torch.transpose(y, dim0=1, dim1=2).contiguous().view(B, H, W, -1).contiguous()
        y = y * F.silu(z)
        out = self.out_proj(y)
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # self.channel_attention1 = ChannelAttentionBlock(hidden_dim)
        # self.channel_attention2 = ChannelAttentionBlock(hidden_dim)

        self.idx = id
        self.patch_size = 2
        self.fft = nn.Parameter(torch.ones((hidden_dim*2, 1, 1, self.patch_size, self.patch_size // 2 + 1)))
        self.fusion_conv = nn.Conv2d(hidden_dim*2, hidden_dim*2, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, input: torch.Tensor):
        ...
        # high_branch_low=self.simple_gate(high_branch_low).permute(0, 2, 3, 1).contiguous()
        high_branch_low = high_branch_low.permute(0, 2, 3, 1).contiguous()

        low = self.drop_path(self.self_attention(self.ln_1(low), self.ln_2(high_branch_low)))

        x = torch.cat([output_high, low.permute(0, 3, 1, 2).contiguous()], dim=1)

        # x = self.fusion_conv(x)
        x = self.channel_attention(x)
        x = x.permute(0, 2, 3, 1).contiguous() + input
