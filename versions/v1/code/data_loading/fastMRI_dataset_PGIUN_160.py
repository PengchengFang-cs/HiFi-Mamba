...
import h5py
from data_loading.subsample import create_mask_for_mask_type
...
class fastMRI_dataset(Dataset):
    ...
    def __init__(self, data_dir, select='FSPD', type='train', acceleration=4, mask_type = 'random', resolution=192, rate=1.0):
        ...
        if mask_type == 'equispaced':
            self.mask = np.load('/scratch/pf2m24/projects/MambaIR/simple_mambair/data_loading/mask_2_160.npy')
        ...
        self.scale_factor = (resolution / 320, resolution / 320)
    ...
    def __getitem__(self, idx):
        ...
        image_rec = zoom(image_rec, zoom=self.scale_factor, order=1)
        ...
        if self.type == 'train':
            ...
            under_image_rec = under_image_rec.transpose(1, 2, 0)
            _, H, W = image_rec.shape
            rnd_h = random.randint(0, max(0, H - self.resolution))
            rnd_w = random.randint(0, max(0, W - self.resolution))
            image_rec = image_rec[rnd_h:rnd_h + self.resolution, rnd_w:rnd_w + self.resolution, :]
            under_image_rec = under_image_rec[rnd_h:rnd_h + self.resolution, rnd_w:rnd_w + self.resolution, :]
            #print(image_rec.shape, under_image_rec.shape)
            mode = random.randint(0, 7)
            image_rec = augment_img(image_rec, mode)
            under_image_rec = augment_img(under_image_rec, mode)
            image_rec = image_rec.transpose(2, 0, 1)
            under_image_rec = under_image_rec.transpose(2, 0, 1)
            #print(image_rec.shape, under_image_rec.shape)
        ...
        # save_image(under_image_rec, "under_sample.png", normalize=True)
        # save_image(image_rec, "gt.png", normalize=True)
        # save_image(mask, "mask.png", normalize=True)
        # save_image(torch.abs(under_sampling), "under_kspace.png", normalize=True)
        return (
                under_image_rec,
                image_rec, # gt
