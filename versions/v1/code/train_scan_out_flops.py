...
def main_worker(args):
    ...
    kspace_data = torch.randn(1, *input_shape).cuda(local_rank)
    input_shape = (1, 320, 320)
    mask_tensor = torch.randn(1, *input_shape).cuda(local_rank)
   
    print(input_tensor.shape, mask_tensor.shape, mask_tensor.shape)

    flops, params = profile(model, inputs=(kspace_data, kspace_data, mask_tensor))
