...
def main_worker(args):
    ...
    pretrained=False,
    drop_path_rate=0.0,
    depths=(2, 2, 6, 2),         # 与Mamba层数匹配
    dims=(96, 192, 384, 768),    # 或调整到相近通道数
    features_only=True
    ).cuda(local_rank)
    ...
    class SimpleDecoder(nn.Module):
        def __init__(self, channels=[96, 192, 384, 768], out_channels=2):
            ...
            self.final_up = nn.Sequential(
                nn.ConvTranspose2d(channels[0], 64, kernel_size=4, stride=4),  # 一次性上采样×4
        ...
        def forward(self, feats):
            # feats: [C1, C2, C3, C4] (list from encoder)
            c1, c2, c3, c4 = feats

            x = self.up3(c4)      # 1/32 → 1/16
            x = x + c3            # skip connection
            x = self.up2(x)       # 1/16 → 1/8
            x = x + c2
            x = self.up1(x)       # 1/8 → 1/4
            x = x + c1
            out = self.final_up(x)  # 1/4 → 1/2 → full res
    ...
    class ConvNextReconNet(nn.Module):
        def __init__(self, model):
            super().__init__()
            self.encoder = model
            self.decoder = SimpleDecoder()  # 与 Mamba 版本完全一致
    ...
    model = ConvNextReconNet(convnext).cuda(local_rank)
    model = DDP(model, device_ids=[local_rank], find_unused_parameters=True)

    num_params = sum(p.numel() for p in model.parameters())
    print(f"Params (trainable): {num_params/1e6:.2f}M")
