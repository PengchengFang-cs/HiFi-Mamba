...
def adjust_learning_rate(optimizer, epoch, total_epochs=72, lr=1e-3, warmup_epochs=5, min_lr=1e-6):
...
def main_worker(args):
    ...
    # 路径和日志
    snapshot_path = f"../model/{args.exp}_{args.labeled_num}_patch2_h100_seed_1337/{args.model}"
    if is_main:
   ...
   # 数据集与采样器
    ...
    if args.resume is not None and os.path.isfile(args.resume):
        ...
        start_epoch = 33  # Manually set, since you've trained 32 epochs
        ...
        logging.info('Nothing Resume. Start new!!!')
    ...
    for epoch in range(start_epoch, start_epoch+ 2):
        ...
        with torch.no_grad():
            for i_batch, sampled_batch in enumerate(tbar):
                ...
                if i_batch > 100 and i_batch <150 and image_path is not None:
                    ...
                    # save_image(image_rec, image_path / f'ori_idx_{i_batch}.png', normalize=True)
                    # save_image(under_image_rec, image_path / f'under_idx_{i_batch}.png', normalize=True)
        ...
        if is_main:
            ...
            # 日志记录
            logging.info(f'[Epoch {epoch}] Val PSNR: {total_psnr:.4f}, SSIM: {total_ssim:.4f}')
            ...
            logging.info(f'nmse: {total_rmse:.4f}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--resume', default='/scratch/pf2m24/projects/MRIRecon/MambaRecon/model/mamba_unrolled_140_patch1_h100_seed_1337_cc359_4x_4x2_p2/mamba_unrolled/mamba_unrolled_best_ssim_model.pth', help='resume from checkpoint')
