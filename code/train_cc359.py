...
DEFAULT_CFG = os.path.join(CODE_DIR, "configs", "vmamba_tiny.yaml")
DEFAULT_OUTPUT_DIR = os.path.join(REPO_ROOT, "outputs", "cc359")
...
def copy_code_snapshot(dst):
    ...
    shutil.copytree(CODE_DIR, dst, ignore=ignore)
...
def main_worker(args):
    ...
    if args.model == "mamba_unrolled":
        ...
        raise ValueError(f"Unknown model: {args.model}. Public release supports mamba_unrolled and mamba_unet.")
    ...
    # ---- paths & logging ----
    run_name = f"{args.exp}_{args.labeled_num}_Patch_{args.patch_size}_{args.name}/{args.model}"
    snapshot_path = os.path.abspath(os.path.join(os.path.expanduser(args.output_dir), run_name))
    if is_main:
        os.makedirs(snapshot_path, exist_ok=True)
        code_dst = os.path.join(snapshot_path, "code")
        if os.path.exists(code_dst):
            shutil.rmtree(code_dst)
        copy_code_snapshot(code_dst)
        logging.basicConfig(
    ...
    trainloader = DataLoader(train_dataset, **train_loader_kwargs)
    valloader = DataLoader(val_dataset, **val_loader_kwargs)
    ...
    for epoch in range(start_epoch, max_epoch + 1):
        ...
        pbar = tqdm(
            trainloader, ncols=120, desc=f'[Train] Epoch {epoch}/{max_epoch}',
        ...
        for i_batch, sampled_batch in enumerate(pbar):
            ...
            fs_target_abs = torch.abs(fs_target[:, 0, :, :] + 1j * fs_target[:, 1, :, :])
            loss = torch.mean(torch.abs(outputs - fs_target_abs))

            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            optimizer.step()
            scheduler.step()  # 每 iter 一次；resume 后不会重新 warmup
            iter_num += 1
            ...
            if is_main and writer:
                ...
                pbar.set_postfix({'loss': f'{loss.item():.4e}', 'lr': f'{lr_:.3e}'})
...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_data_dir', type=str, required=True, help='Path to CC359 training directory.')
    parser.add_argument('--val_data_dir', type=str, required=True, help='Path to CC359 validation directory.')
    parser.add_argument('--output_dir', type=str, default=DEFAULT_OUTPUT_DIR)
    parser.add_argument('--root_path', type=str, default='')
    parser.add_argument('--exp', type=str, default='mamba_unrolled')
    parser.add_argument('--dataset', type=str, default='cc359')
    parser.add_argument('--model', type=str, default='mamba_unrolled', choices=['mamba_unrolled', 'mamba_unet'])
    ...
    parser.add_argument('--name', type=str, default='cc359_8x')
    parser.add_argument('--acceleration', type=int, default=8)
    parser.add_argument('--mask_type', type=str, default='equispaced', choices=['equispaced', 'random'])
